...
static void test_tokenize_multi_tokens(void) {
    ...
    assert_int_eq(tokens[0]->ty, TK_OPEN_CURLY);
    assert_int_eq(tokens[1]->ty, TK_SPACE);
    assert_int_eq(tokens[2]->ty, TK_NUMBER);
    assert_int_eq(tokens[2]->u.number, 123);
    assert_int_eq(tokens[3]->ty, TK_SPACE);
    assert_int_eq(tokens[4]->ty, TK_EXECUTABLE_NAME);
    assert_str_eq(tokens[4]->u.name, "foo");
    assert_int_eq(tokens[5]->ty, TK_SPACE);
    assert_int_eq(tokens[6]->ty, TK_LITERAL_NAME);
    assert_str_eq(tokens[6]->u.name, "bar");
    assert_int_eq(tokens[7]->ty, TK_SPACE);
    assert_int_eq(tokens[8]->ty, TK_CLOSE_CURLY);
}
